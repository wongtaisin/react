var province = ["广东省", "天津市", "河北省", "河南省", "山西省", "山东省", "内蒙古自治区", "辽宁省", "吉林省", "黑龙江省", "上海市", "江苏省", "浙江省", "福建省", "江西省", "安徽省", "湖北省", "湖南省", "北京市", "广西壮族自治区", "海南省", "重庆市", "四川省", "贵州省", "云南省", "西藏自治区", "陕西省", "甘肃省", "青海省", "宁夏回族自治区", "新疆维吾尔自治区", "台湾省", "香港特别行政区", "澳门特别行政区"]
var city = [
...
var areaContainerId = "#areaList"

if ($(areaContainerId).length == 0) {
	var tpl = ` <section id="areaLayer" class="express-area-box">
					<header>
							<h3>选择地区</h3>
							<i></i>
							<a id="backUp" class="back" href="javascript:void(0)">返回</a>
							<a id="closeArea" class="close" href="javascript:void(0)"><i class="iconfont icon-guanbifuzhi"></i></a>
							<a id="close" class="close" href="javascript:void(0)"></a>
					</header>
					<article id="areaBox">
...
function intProvince() {
	...
	for (var i = 0; i < province.length; i++) {
		areaCont += '<li onClick="selectP(' + i + ');" data-province=' + province[i] + '>' + province[i] + '</li>'
	}
...
var dismissArea = function () {
	$("#areaMask").fadeOut()
	$("#areaLayer").animate({ "bottom": "-100%" })
	intProvince()
}

var expressArea, areaCont, areaList = $(areaContainerId), areaTop = areaList.offset().top
/*选择省份*/
function selectP(p) {
	areaCont = ""
	areaList.html("")
...
function selectD(p, c, d) {
	...
	$('.js-district').val(district[p][c][d])
}
...
var AddressSelect = function () {
	...
	module.show = function () {
		$("#areaMask").fadeIn()
		$("#areaLayer").animate({ "bottom": 0 }).css({ 'display': 'block' })
	}
...
$(function () {
	var AddressSelector = AddressSelect()
	intProvince()
	$("#expressArea").on('click', function () {
		AddressSelector.show()
	})
	$("#close, #areaMask").on('click', function () {
		$('#areaLayer, #areaMask').hide()
	})
}) 
